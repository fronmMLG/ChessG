...
function updatePGN(startingSquareId,destinationSquareId,whiteTurn,promotedTo="") {
  ...
  function recreateHTMLFromPGN(pgn) {
    ...
    for (let i = 0; i < moveArray.length; i++) {
        ...
        // Добавляем обработчик клика для просмотра партии
        newMove.addEventListener("click", function () {
            viewedIndex = moveId + 1;
            updatePosition();
            highlightMove(viewedIndex);
            allowMovement = (viewedIndex === positionArray.length);
        });
...
function moveForward() {
  ...
  function moveToEnd() {
    if (positionArray.length > 0) {
        viewedIndex = positionArray.length; // Было: viewedIndex = positionArray.length;
        updatePosition();
        highlightMove(viewedIndex);
        allowMovement = true;
    }
...
function highlightMove(viewedIndex) {
  document.querySelectorAll(".playerMove").forEach(element => {
...
function updatePosition() {
  console.log("updatePosition вызван, viewedIndex:", viewedIndex);

  if (positionArray.length === 0) {
      console.log("Ошибка: positionArray пустой! Нечего обновлять.");
      return;
  }

  if (viewedIndex < 0) {
      console.log("viewedIndex меньше 0! Исправляем на 0.");
      viewedIndex = 0;
  }

  if (viewedIndex >= positionArray.length) {
      console.log("viewedIndex больше чем positionArray.length - 1! Исправляем.");
      viewedIndex = positionArray.length - 1;
  }

  let viewedFEN = positionArray[viewedIndex];
  if (!viewedFEN) {
      console.log("viewedFEN всё ещё undefined! Проверяем positionArray:", positionArray);
      return;
  }

  console.log("Перед загрузкой FEN:", viewedFEN);
...
function loadPositionFromFEN(fen) {
  console.log("loadPositionFromFEN вызван, FEN:", fen);
  
  if (!fen) {
      console.log("FEN пустой! Не обновляем доску.");
      return;
  }

  clearBoard(); // Очистка доски перед обновлением
  const pieceMap = {
  ...
  rows.forEach((row, rowIndex) => {
      ...
      for (const char of row) {
          if (isNaN(char)) {
              ...
              console.log(`Ставим ${color} ${pieceType} на ${squareId}`);
...
(function() {
  ...
  console.log = function(message) {
    originalLog(message); // Вызов оригинального console.log
    const consoleOutput = document.getElementById('consoleOutput');
    consoleOutput.textContent = message; // Очищаем и выводим только последнее сообщение
  };
})();
...
(function() {
  ...
  // Создаём контейнер для вывода сообщения
  const consoleContainer = document.createElement('div');
  consoleContainer.id = 'consoleOutput-name';
  document.body.appendChild(consoleContainer);

  console.log = function(message) {
    originalLog(message); // Вызов оригинального console.log

    // Проверяем, что в контейнере нет сообщений
    if (consoleContainer.children.length === 0) {
      const messageElement = document.createElement('h3');
      messageElement.textContent = message;
      consoleContainer.appendChild(messageElement); // Добавляем только первое сообщение
    }
  };
})();
...
document.getElementById("pgnUpload").addEventListener("change", function (event) {
  ...
  if (file.size === 0) {
      console.log("Ошибка: Файл пустой!");
      return;
  }
  ...
  reader.onload = function (e) {
      ...
      console.log("Файл загружен полностью:\n", result.substring(0, 200));

      if (!result) {
          console.log("Ошибка: содержимое файла пустое!");
          return;
      }

      console.log("Передача в processPGN...");
      processPGN(result); // Передаём содержимое файла в обработчик
  };
...
function readFileAsEncoding(file, encoding) {
  const reader = new FileReader();

  reader.onload = function (e) {
      let result = e.target.result;
      console.log(`Файл загружен в кодировке ${encoding}, первые 200 символов:\n`, result.substring(0, 200));

      if (!result.trim()) {
          console.log(`Ошибка: Файл пуст даже в ${encoding}!`);
          return;
      }
...
// Функция обработки PGN
function processPGN(pgnText) {
  console.log("Оригинальный PGN перед обработкой:\n", pgnText);

  let movesString = extractMovesFromPGN(pgnText);
  console.log("PGN после очистки:", movesString);

  positionArray = [];
  viewedIndex = 0;
  let boardSquaresCopy = deepCopyArray(boardSquaresArray);

  let moveList = movesString.match(/[a-h][1-8][a-h][1-8][qrbn]?|O-O-O|O-O/g) || [];
  console.log("moveList после парсинга:", moveList);

  moveList.forEach(move => {
      let fromTo = move.match(/[a-h][1-8][a-h][1-8]/);
      let promotion = move.match(/[a-h][1-8][a-h][1-8][qrbn]/);

      if (fromTo) {
          let from = fromTo[0].substring(0, 2);
          let to = fromTo[0].substring(2, 4);
          let promotedTo = promotion ? promotion[0][4] : "blank";

          console.log(`Обрабатываем ход: ${move}, from: ${from}, to: ${to}, promotion: ${promotedTo}`);

          updateBoardSquaresArray(from, to, boardSquaresCopy, promotedTo);
          let newFEN = generateFEN(boardSquaresCopy);

          console.log(`FEN после ${move}:`, newFEN);
          positionArray.push(newFEN);
      }
  });

  console.log("positionArray после обработки:", positionArray);

  if (positionArray.length > 0) {
      viewedIndex = positionArray.length - 1;
      console.log("viewedIndex исправлен:", viewedIndex);
      updatePosition();
  } else {
      console.log("Ошибка: positionArray всё ещё пустой!");
  }

  recreateHTMLFromPGN(movesString);
}
...
// Функция для извлечения ходов из PGN
function extractMovesFromPGN(pgn) {
  console.log("Исходный PGN перед очисткой:\n", pgn);

  let cleanedPGN = pgn
    .replace(/\[[^\]]*\]/g, "")  // Убираем заголовки PGN
    .replace(/\d+\./g, "")       // Убираем номера ходов
    .replace(/(\r\n|\n|\r)/gm, " ") // Убираем переносы строк
    .replace(/1-0|0-1|1\/2-1\/2/g, "") // Убираем результат игры
    .trim();

  console.log("PGN после очистки:\n", cleanedPGN);
  return cleanedPGN;
}
