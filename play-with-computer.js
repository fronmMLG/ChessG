...
const botInfoElement = document.getElementById("bot-info");
...
function flipBoard() {
  let flipped = chessBoard.style.transform.includes("scale(-1, -1)");
  chessBoard.style.transform = flipped 
    ? "translate(-50%, -50%) scale(1, 1)" 
    : "translate(-50%, -50%) scale(-1, -1)";
  Array.from(document.getElementsByClassName("piece")).forEach(div => {
    div.style.transform = flipped ? "scale(1, 1)" : "scale(-1, -1)";
  });
  ...
  if (isEngineWhite) getBestMove(currentPosition, playBestMove);
}
...
function getBestMove(fen,callback) {
   let engine = new Worker("/chessapp/node_modules/stockfish/src/stockfish-nnue-16.js");
...
function updateBotInfo() {
  botInfoElement.textContent = `Bot: Level ${selectedLevel}`;
}
level.addEventListener("change", function () {
  selectedLevel = this.value;
  updateBotInfo();
